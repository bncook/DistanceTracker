...
class DistanceTracker:
    ...
    def find_position(self, focal_length, real_height):
        """
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            dist, offset = self.find_position(self.focal_length, self.real_height)
            ...
            if dist != 0:
                ...
                cv_marked_image = cv2.circle(self.image, (center_x, center_y), 15, (0,0,255), 15)
    ...
    def callback(self, ros_data):
        bridge = CvBridge()
        if self.rectified:
            self.image = bridge.imgmsg_to_cv2(ros_data, desired_encoding='passthrough')
        else:
...
if __name__ == '__main__':
    ...
    if (len(sys.argv) > 1):
        if ("--rectified" in sys.argv):
            ...
            rospy.Subscriber('/raspicam_node/image_rect_color', Image, tracker.callback)
            print("Beginning position tracker with rectified image at 10hz\n")
    else:
